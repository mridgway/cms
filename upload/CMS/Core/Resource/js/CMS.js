...
CMS._loadedScripts = new Array();
CMS._callbackQueue = new Array();
CMS.Use = function (scripts, callback) {
    // Translate 'Module/Namespace.ClassName' into a path
    for (i in scripts) {
        if (scripts[i][0] != '/' && scripts[i].substr(0,4)!= 'http') {
            var scriptParts = scripts[i].split('/');
            var scriptPath = '/resources/'+scriptParts[0].toLowerCase()+'/js/'+scriptParts[1].substring(4).replace(/\./g, '/')+'.js';
    ...
    CMS._callbackQueue.push({
        callback: callback,
        dependencies: scripts
    });
    for (var i=0; i<scriptsLength; ++i) {
        var script = scripts[i];
        if (CMS._loadedScripts[script] == false) {
            
        } else if ((typeof CMS._loadedScripts[script] == 'undefined')) {
            CMS._loadedScripts[script] = false;
            scriptsToLoad.push(script);
        }
    }

    var self = this;
    var completionCallback = function (scripts) {
        var scriptsToLoadLength = scriptsToLoad.length;

        for (var i=0; i<scriptsToLoadLength; ++i) {
            CMS._loadedScripts[scriptsToLoad[i]] = true;
        }
        for (script in CMS._loadedScripts) {
            if (!CMS._loadedScripts[script]) {
                return;
            }
        }
        ...
        while (func = CMS._callbackQueue.pop()) {
            ...
            for (script in func.dependencies) {
                if (!(func.dependencies[script] in CMS._loadedScripts)
                    || !CMS._loadedScripts[func.dependencies[script]]) {
...
CMS.ScriptQueue = Class.extend({
    ...
    init: function (scripts, callback) {
        ...
        this.scriptCount = scripts.length;
        for (var i = 0; i < this.scriptCount; ++i) {
            this._load(scripts[i]);
        }
    },
