...
namespace Modo\Controller\Plugin;
...
class Predispatch extends \Zend_Controller_Plugin_Abstract
{
    ...
    {
        if (!$request->isDirect()) {
            $pageRouteService = \Zend_Registry::get('doctrine')->getRepository('Core\Model\PageRoute');
            $pageRoute = $pageRouteService->getPageRoute($request->getRouteId(), $request->getSerializedParams());
            if (null === $pageRoute) {
